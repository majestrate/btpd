...
{
    struct blog *log = toxn_calloc(1, sizeof(*log));
    TOXNQ_INIT(&log->records);
    TOXNQ_INSERT_HEAD(&pc->logs, log, entry);
}
...
piece_log_hashes(struct piece *pc)
{
    ...
    struct blog *log = TOXNQ_FIRST(&pc->logs);
    log->hashes = toxn_malloc(20 * pc->nblocks);
...
piece_log_free(struct piece *pc, struct blog *log)
{
    ...
    TOXNQ_FOREACH_MUTABLE(r, &log->records, entry, rnext) {
        mp_drop(r->mp, pc->n);
        free(r);
    }
    if (log->hashes != NULL)
...
piece_kill_logs(struct piece *pc)
{
    ...
    TOXNQ_FOREACH_MUTABLE(log, &pc->logs, entry, lnext)
...
{
    struct blog *log = TOXNQ_FIRST(&pc->logs);
    struct blog_record *r = TOXNQ_FIRST(&log->records);
    struct meta_peer *culprit = NULL;

    if (r == TOXNQ_LAST(&log->records, blog_record_tq)) {
        unsigned i;
    ...
    if (culprit != NULL) {
        ...
        TOXNQ_REMOVE(&pc->logs, log, entry);
        piece_log_free(pc, log);
    } else {
        TOXNQ_FOREACH(r, &log->records, entry) {
            if (r->mp->p != NULL) {
                if (pc->n->endgame)
...
piece_log_good(struct piece *pc)
{
    ...
    struct blog *log = TOXNQ_FIRST(&pc->logs), *bad = TOXNQ_NEXT(log, entry);
    ...
    while (bad != NULL) {
        TOXNQ_FOREACH(r, &bad->records, entry) {
            int culprit = 0;
...
piece_log_block(struct piece *pc, struct peer *p, uint32_t begin)
{
    ...
    TOXNQ_FOREACH(r, &log->records, entry)
        if (r->mp == p->mp)
            break;
    if (r == NULL) {
        r = toxn_calloc(1, sizeof(*r) + ceil(pc->nblocks / 8.0));
        r->mp = p->mp;
        mp_hold(r->mp);
        TOXNQ_INSERT_HEAD(&log->records, r, entry);
    } else {
        TOXNQ_REMOVE(&log->records, r, entry);
        TOXNQ_INSERT_HEAD(&log->records, r, entry);
    }
...
piece_alloc(struct net *n, uint32_t index)
{
    ...
    TOXNQ_INSERT_TAIL(&n->getlst, pc, entry);
    return pc;
}
...
piece_free(struct piece *pc)
{
    ...
    TOXNQ_REMOVE(&pc->n->getlst, pc, entry);
    TOXNQ_FOREACH_MUTABLE(req, &pc->reqs, blk_entry, next) {
        nb_drop(req->msg);
        free(req);
    }
    piece_kill_logs(pc);
...
dl_should_enter_endgame(struct net *n)
{
    ...
    if (cm_pieces(n->tp) + n->npcs_busy == n->tp->npieces) {
        should = 1;
        struct piece *pc;
        TOXNQ_FOREACH(pc, &n->getlst, entry) {
            if (!piece_full(pc)) {
                should = 0;
                break;
            }
...
dl_piece_insert_eg(struct piece *pc)
{
    ...
    if (pc->nblocks == pc->ngot)
        TOXNQ_INSERT_TAIL(getlst, pc, entry);
    else {
        unsigned r = pc->nreqs / (pc->nblocks - pc->ngot);
        struct piece *it;
        TOXNQ_FOREACH(it, getlst, entry) {
            if ((it->nblocks == it->ngot
                    || r < it->nreqs / (it->nblocks - it->ngot))) {
                TOXNQ_INSERT_BEFORE(it, pc, entry);
                break;
            }
        }
        if (it == NULL)
...
dl_enter_endgame(struct net *n)
{
    ...
    toxn_log(TOXN_L_POL, "Entering end game\n");
    n->endgame = 1;
    ...
    TOXNQ_FOREACH(pc, &n->getlst, entry) {
        ...
        pc->nbusy = 0;
        pc->eg_reqs = toxn_calloc(pc->nblocks, sizeof(struct net_buf *));
        TOXNQ_FOREACH(req, &pc->reqs, blk_entry) {
...
{
    struct peer *p;
    TOXNQ_FOREACH(p, &pc->n->peers, p_entry)
...
{
    toxn_log(TOXN_L_POL, "Started on piece %u.\n", index);
...
dl_new_request(struct peer *p, struct piece *pc, struct net_buf *msg)
{
    ...
    struct block_request *req = toxn_malloc(sizeof(*req));
    req->p = p;
    req->msg = msg;
    nb_hold(req->msg);
    TOXNQ_INSERT_TAIL(&pc->reqs, req, blk_entry);
    pc->nreqs++;
    if (!pc->n->endgame) {
...
{
    while (p->nreqs_out > 0) {
        struct block_request *req = TOXNQ_FIRST(&p->my_reqs);
        struct piece *pc = dl_find_piece(p->n, nb_get_index(req->msg));
        int was_full = piece_full(pc);

        while (req != NULL) {
            struct block_request *next = TOXNQ_NEXT(req, p_entry);
            ...
            p->nreqs_out--;
            TOXNQ_REMOVE(&pc->reqs, req, blk_entry);
            nb_drop(req->msg);
            free(req);
            pc->nreqs--;

            while (next != NULL && nb_get_index(next->msg) != pc->index)
                next = TOXNQ_NEXT(next, p_entry);
            req = next;
        }
...
dl_assign_requests_eg(struct peer *p)
{
    ...
    struct piece *pc = TOXNQ_FIRST(&n->getlst);
    while (!peer_laden(p) && pc != NULL) {
        struct piece *next = TOXNQ_NEXT(pc, entry);
        if (peer_requestable(p, pc->index) && pc->nblocks != pc->ngot) {
            dl_piece_assign_requests_eg(pc, p);
            TOXNQ_REMOVE(&n->getlst, pc, entry);
            TOXNQ_INSERT_HEAD(&tmp, pc, entry);
        }
        pc = next;
    }
