...
TOXNQ_HEAD(ai_ctx_tq, ai_ctx);

static struct ai_ctx_tq m_aiq = TOXNQ_HEAD_INITIALIZER(m_aiq);
static pthread_mutex_t m_aiq_lock;
static pthread_cond_t m_aiq_cond;

struct ai_ctx *
toxn_addrinfo(const char *node, uint16_t port, struct addrinfo *hints,
    void (*cb)(void *, int, struct addrinfo *), void *arg)
{
    struct ai_ctx *ctx = toxn_calloc(1, sizeof(*ctx));
    ctx->hints = *hints;
    ctx->cb = cb;
    ctx->arg = arg;    
    ...
    TOXNQ_INSERT_TAIL(&m_aiq, ctx, entry);
...
void
toxn_addrinfo_cancel(struct ai_ctx *ctx)
{
    ctx->cancel = 1;
}
...
addrinfo_td(void *arg)
{
    ...
    while (1) {
        pthread_mutex_lock(&m_aiq_lock);
        while (TOXNQ_EMPTY(&m_aiq))
            pthread_cond_wait(&m_aiq_cond, &m_aiq_lock);
        ctx = TOXNQ_FIRST(&m_aiq);
        TOXNQ_REMOVE(&m_aiq, ctx, entry);
        pthread_mutex_unlock(&m_aiq_lock);
