...
struct start_test_data {
    ...
    uint32_t start;
    TOXNQ_ENTRY(start_test_data) entry;
};

TOXNQ_HEAD(std_tq, start_test_data);

static struct std_tq m_startq = TOXNQ_HEAD_INITIALIZER(m_startq);
...
test_piece(struct torrent *tp, uint32_t piece, int *ok)
{
    ...
    if ((err = bts_sha(tp->cm->rds, piece * tp->piece_length,
        ...
        toxn_log(TOXN_L_ERROR, "io error on '%s' (%s).\n",
...
cm_write_done(struct torrent *tp)
{
    ...
    cm->wrs = NULL;
    if (err && !cm->error) {
        toxn_log(TOXN_L_ERROR, "error closing write stream for '%s' (%s).\n",
...
cm_stop(struct torrent *tp)
{
    ...
    if (cm->state == CM_STARTING) {
        ...
        TOXNQ_FOREACH(std, &m_startq, entry)
            if (std->tp == tp) {
                TOXNQ_REMOVE(&m_startq, std, entry);
                free(std->fts);
                free(std);
                break;
            }
...
cm_create(struct torrent *tp, const char *mi)
{
    ...
    struct content *cm = toxn_calloc(1, sizeof(*cm));
    cm->bppbf = ceil((double)tp->piece_length / (1 << 17));
    cm->pos_field = toxn_calloc(pfield_size, 1);
        ...
        if (stat(path, &sb) == -1) {
            if (errno == ENOENT) {
                ...
                if (errno != 0 || close(fd) != 0) {
                    toxn_log(TOXN_L_ERROR, "failed to create '%s' (%s).\n",
                        path, strerror(errno));
                    return errno;
                }
                goto again;
            } else {
                toxn_log(TOXN_L_ERROR, "failed to stat '%s' (%s).\n",
                    path, strerror(errno));
                return errno;
            }
        } else if (sb.st_size > tp->files[i].length) {
            if (truncate(path, tp->files[i].length) != 0) {
                toxn_log(TOXN_L_ERROR, "failed to truncate '%s' (%s).\n",
                    path, strerror(errno));
                return errno;
            }
            goto again;
...
startup_test_end(struct torrent *tp, int unclean)
{
    ...
    if (unclean) {
        struct start_test_data *std = TOXNQ_FIRST(&m_startq);
        TOXNQ_REMOVE(&m_startq, std, entry);
        for (int i = 0; i < tp->nfiles; i++)
...
startup_test_run(void)
{
    ...
    if (!TOXNQ_EMPTY(&m_startq))
        toxn_timer_add(&m_workev, (& (struct timespec) { 0, 0 }));
}
...
startup_test_begin(struct torrent *tp, struct file_time_size *fts)
{
    ...
    while (piece < tp->npieces && !has_bit(cm->pos_field, piece))
        piece++;
    if (piece < tp->npieces) {
        struct start_test_data *std = toxn_calloc(1, sizeof(*std));
        std->tp = tp;
        std->start = piece;
        std->fts = fts;
        TOXNQ_INSERT_TAIL(&m_startq, std, entry);
        if (std == TOXNQ_FIRST(&m_startq))
            toxn_timer_add(&m_workev, (& (struct timespec) { 0, 0 }));
    } else {
        free(fts);
...
cm_start(struct torrent *tp, int force_test)
{
    ...
    if ((errno =
        ...
        toxn_log(TOXN_L_ERROR, "failed to open stream for '%s' (%s).\n",
            torrent_name(tp), strerror(errno));
        cm_on_error(tp);
        return;
    }

    fts = toxn_calloc(tp->nfiles, sizeof(*fts));
