...
mp_create(void)
{
    return toxn_calloc(1, sizeof(struct meta_peer));
}
...
peer_kill(struct peer *p)
{
    ...
    toxn_log(TOXN_L_CONN, "killed peer %p\n", p);

    if (p->mp->flags & PF_ATTACHED) {
        TOXNQ_REMOVE(&p->n->peers, p, p_entry);
        p->n->npeers--;
        if (p->n->active) {
            ul_on_lost_peer(p);
            dl_on_lost_peer(p);
        ...
        TOXNQ_REMOVE(&net_unattached, p, p_entry);
    if (p->mp->flags & PF_ON_READQ)
        TOXNQ_REMOVE(&net_bw_readq, p, rq_entry);
    if (p->mp->flags & PF_ON_WRITEQ)
        TOXNQ_REMOVE(&net_bw_writeq, p, wq_entry);

    toxn_ev_del(&p->ioev);
    close(p->sd);

    nl = TOXNQ_FIRST(&p->outq);
    while (nl != NULL) {
        struct nb_link *next = TOXNQ_NEXT(nl, entry);
        nb_drop(nl->nb);
        free(nl);
        nl = next;
    }
...
{
    struct nb_link *nl = toxn_calloc(1, sizeof(*nl));
    nl->nb = nb;
    nb_hold(nb);

    if (TOXNQ_EMPTY(&p->outq)) {
        assert(p->outq_off == 0);
        toxn_ev_enable(&p->ioev, EV_WRITE);
        p->t_wantwrite = toxn_seconds;
    }
    TOXNQ_INSERT_TAIL(&p->outq, nl, entry);
}
...
{
    if (!(nl == TOXNQ_FIRST(&p->outq) && p->outq_off > 0)) {
        TOXNQ_REMOVE(&p->outq, nl, entry);
        ...
        if (TOXNQ_EMPTY(&p->outq)) {
            if (p->mp->flags & PF_ON_WRITEQ) {
                ...
                toxn_ev_disable(&p->ioev, EV_WRITE);
        }
        return 1;
    } else
        return 0;
...
peer_sent(struct peer *p, struct net_buf *nb)
{
    switch (nb->type) {
    case NB_KEEPALIVE:
        toxn_log(TOXN_L_MSG, "sent keepalive to %p\n", p);
        break;
    case NB_CHOKE:
        toxn_log(TOXN_L_MSG, "sent choke to %p\n", p);
        break;
    case NB_UNCHOKE:
        toxn_log(TOXN_L_MSG, "sent unchoke to %p\n", p);
        p->mp->flags &= ~PF_NO_REQUESTS;
        break;
    case NB_INTEREST:
        toxn_log(TOXN_L_MSG, "sent interest to %p\n", p);
        break;
    case NB_UNINTEREST:
        toxn_log(TOXN_L_MSG, "sent uninterest to %p\n", p);
        break;
    case NB_HAVE:
        toxn_log(TOXN_L_MSG, "sent have(%u) to %p\n",
            nb_get_index(nb), p);
        break;
    case NB_BITFIELD:
        toxn_log(TOXN_L_MSG, "sent bitfield to %p\n", p);
        break;
    case NB_REQUEST:
        toxn_log(TOXN_L_MSG, "sent request(%u,%u,%u) to %p\n",
            nb_get_index(nb), nb_get_begin(nb), nb_get_length(nb), p);
        break;
    case NB_PIECE:
        toxn_log(TOXN_L_MSG, "sent piece(%u,%u,%u) to %p\n",
            nb_get_index(nb), nb_get_begin(nb), nb_get_length(nb), p);
        break;
    case NB_CANCEL:
        toxn_log(TOXN_L_MSG, "sent cancel(%u,%u,%u) to %p\n",
            nb_get_index(nb), nb_get_begin(nb), nb_get_length(nb), p);
        break;
    case NB_TORRENTDATA:
        toxn_log(TOXN_L_MSG, "sent data to %p\n", p);
        assert(p->npiece_msgs > 0);
        p->npiece_msgs--;
        break;
    case NB_MULTIHAVE:
        toxn_log(TOXN_L_MSG, "sent multihave to %p\n", p);
        break;
    case NB_BITDATA:
        toxn_log(TOXN_L_MSG, "sent bitdata to %p\n", p);
        break;
    case NB_SHAKE:
        toxn_log(TOXN_L_MSG, "sent shake to %p\n", p);
        break;
    }
}
...
peer_request(struct peer *p, struct block_request *req)
{
    ...
    p->nreqs_out++;
    TOXNQ_INSERT_TAIL(&p->my_reqs, req, p_entry);
    peer_send(p, req->msg);
}
...
peer_requested(struct peer *p, uint32_t piece, uint32_t block)
{
    ...
    TOXNQ_FOREACH(req, &p->my_reqs, p_entry)
...
{
    TOXNQ_REMOVE(&p->my_reqs, req, p_entry);
    p->nreqs_out--;

    int removed = 0;
    struct nb_link *nl;
    TOXNQ_FOREACH(nl, &p->outq, entry) {
        if (nl->nb == req->msg) {
...
{
    struct nb_link *nl = TOXNQ_FIRST(&p->outq);
    while (nl != NULL) {
        struct nb_link *next = TOXNQ_NEXT(nl, entry);
        if (nl->nb->type == NB_PIECE) {
            struct nb_link *data = next;
            next = TOXNQ_NEXT(next, entry);
            if (peer_unsend(p, nl))
                peer_unsend(p, data);
        }
        nl = next;
...
peer_want(struct peer *p, uint32_t index)
{
    ...
    if (p->nwant == 1) {
        ...
        if (p->nreqs_out == 0) {
            ...
            int unsent = 0;
            struct nb_link *nl = TOXNQ_LAST(&p->outq, nb_tq);
...
{
    struct peer *p = toxn_calloc(1, sizeof(*p));
    ...
    p->t_created = toxn_seconds;
    p->t_lastwrite = toxn_seconds;
    p->t_nointerest = toxn_seconds;
    TOXNQ_INIT(&p->my_reqs);
    TOXNQ_INIT(&p->outq);

    peer_set_in_state(p, SHAKE_PSTR, 28);

    toxn_ev_new(&p->ioev, p->sd, EV_READ, net_io_cb, p);

    TOXNQ_INSERT_TAIL(&net_unattached, p, p_entry);
    net_npeers++;
    return p;
}
...
{
    toxn_log(TOXN_L_MSG, "received keep alive from %p\n", p);
}
...
peer_on_shake(struct peer *p)
{
    ...
    for (i = 0; i < 20 && isprint(p->mp->id[i]); i++)
        printid[i] = p->mp->id[i];
    printid[i] = '\0';
    toxn_log(TOXN_L_MSG, "received shake(%s) from %p\n", printid, p);
    p->piece_field = toxn_calloc(1, (int)ceil(p->n->tp->npieces / 8.0));
    ...
    TOXNQ_REMOVE(&net_unattached, p, p_entry);
    TOXNQ_INSERT_HEAD(&p->n->peers, p, p_entry);
...
{
    toxn_log(TOXN_L_MSG, "received choke from %p\n", p);
...
{
    toxn_log(TOXN_L_MSG, "received unchoke from %p\n", p);
    if ((p->mp->flags & PF_P_CHOKE) == 0)
        return;
    else {
        p->mp->flags &= ~PF_P_CHOKE;
...
{
    toxn_log(TOXN_L_MSG, "received interest from %p\n", p);
...
{
    toxn_log(TOXN_L_MSG, "received uninterest from %p\n", p);
    if ((p->mp->flags & PF_P_WANT) == 0)
        return;
    else {
        p->mp->flags &= ~PF_P_WANT;
...
{
    toxn_log(TOXN_L_MSG, "received have(%u) from %p\n", index, p);
...
{
    toxn_log(TOXN_L_MSG, "received bitfield from %p\n", p);
    assert(p->npieces == 0);
    ...
    uint32_t length, const char *data)
{
    ...
    TOXNQ_FOREACH(req, &p->my_reqs, p_entry)
        if ((nb_get_begin(req->msg) == begin &&
            ...
            break;
    if (req != NULL) {
        toxn_log(TOXN_L_MSG, "received piece(%u,%u,%u) from %p\n",
        ...
        toxn_log(TOXN_L_MSG, "discarded piece(%u,%u,%u) from %p\n",
            index, begin, length, p);
}
...
peer_on_request(struct peer *p, uint32_t index, uint32_t begin,
    uint32_t length)
{
    toxn_log(TOXN_L_MSG, "received request(%u,%u,%u) from %p\n",
        index, begin, length, p);
...
peer_on_cancel(struct peer *p, uint32_t index, uint32_t begin,
    uint32_t length)
{
    toxn_log(TOXN_L_MSG, "received cancel(%u,%u,%u) from %p\n",
        index, begin, length, p);
    struct nb_link *nl;
    TOXNQ_FOREACH(nl, &p->outq, entry)
        if (nl->nb->type == NB_PIECE
            ...
            struct nb_link *data = TOXNQ_NEXT(nl, entry);
            if (peer_unsend(p, nl))
                peer_unsend(p, data);
            break;
        }
...
peer_on_tick(struct peer *p)
{
    ...
    if (p->mp->flags & PF_ATTACHED) {
        if (TOXNQ_EMPTY(&p->outq)) {
            if (toxn_seconds - p->t_lastwrite >= 120)
                peer_keepalive(p);
        } else if (toxn_seconds - p->t_wantwrite >= 60) {
            toxn_log(TOXN_L_CONN, "write attempt timed out.\n");
            goto kill;
        }
        if ((cm_full(p->n->tp) && !(p->mp->flags & PF_P_WANT) &&
                toxn_seconds - p->t_nointerest >= 600)) {
            toxn_log(TOXN_L_CONN, "no interest for 10 minutes.\n");
            goto kill;
        }
    } else if (toxn_seconds - p->t_created >= 60) {
            toxn_log(TOXN_L_CONN, "hand shake timed out.\n");
            goto kill;
    }
    return;
kill:
...
peer_bad_piece(struct peer *p, uint32_t index)
{
    if (p->npcs_bad == 0) {
        ...
        p->bad_field = toxn_calloc(ceil(p->n->tp->npieces / 8.0), 1);
    }
    ...
    p->suspicion++;
    if (p->suspicion == 3) {
        toxn_log(TOXN_L_BAD, "suspect peer %p.\n", p);
...
peer_good_piece(struct peer *p, uint32_t index)
{
    ...
    p->suspicion = 0;
    if (p->mp->flags & PF_SUSPECT) {
        toxn_log(TOXN_L_BAD, "unsuspect peer %p.\n", p);
        p->mp->flags &= ~PF_SUSPECT;
        if (p->nwant > 0) {
