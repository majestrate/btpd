...
static struct torrent_tq m_torrents = TOXNQ_HEAD_INITIALIZER(m_torrents);
...
torrent_kill(struct torrent *tp)
{
    ...
    if (tp->state == T_GHOST)
        m_nghosts--;
    m_ntorrents--;
    TOXNQ_REMOVE(&m_torrents, tp, entry);
    if (tp->delete)
        tlib_kill(tp->tl);
    else
        tp->tl->tp = NULL;
    ...
    if (m_savetp == tp)
        if ((m_savetp = TOXNQ_NEXT(tp, entry)) == NULL)
            m_savetp = TOXNQ_FIRST(&m_torrents);
    free(tp);
}
...
torrent_start(struct tlib *tl)
{
    ...
    toxn_log(TOXN_L_TOXN, "Starting torrent '%s'.\n", torrent_name(tp));
    tr_create(tp, mi);
    tl->tp = tp;
    net_create(tp);
    cm_create(tp, mi);
    TOXNQ_INSERT_TAIL(&m_torrents, tp, entry);
    m_ntorrents++;
    cm_start(tp, 0);
    free(mi);
    if (m_ntorrents == 1) {
        m_tsave = toxn_seconds + SAVE_INTERVAL;
        m_savetp = tp;
    }
    return IPC_OK;
}
...
{
    toxn_log(TOXN_L_TOXN, "Stopped torrent '%s'.\n", torrent_name(tp));
    tp->state = T_GHOST;
    if (tp->delete)
        tlib_del(tp->tl);
    else
...
torrent_on_tick(struct torrent *tp)
{
    ...
    case T_LEECH:
        if (cm_full(tp)) {
            struct peer *p, *next;
            tp->state = T_SEED;
            toxn_log(TOXN_L_TOXN, "Finished downloading '%s'.\n",
                torrent_name(tp));
            tr_complete(tp);
...
torrent_on_tick_all(void)
{
    ...
    TOXNQ_FOREACH_MUTABLE(tp, &m_torrents, entry, next)
        torrent_on_tick(tp);

    if (m_savetp != NULL && m_tsave <= toxn_seconds) {
        if (m_savetp->state == T_LEECH || m_savetp->state == T_SEED) {
            tlib_update_info(m_savetp->tl, 1);
            if ((m_savetp = TOXNQ_NEXT(m_savetp, entry)) == NULL)
                m_savetp = TOXNQ_FIRST(&m_torrents);
            if (m_ntorrents > 0)
                m_tsave = toxn_seconds +
