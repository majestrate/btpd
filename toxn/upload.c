...
static struct peer_tq m_peerq = TOXNQ_HEAD_INITIALIZER(m_peerq);
...
choke_do(void)
{
    if (m_max_uploads < 0) {
        struct peer *p;
        TOXNQ_FOREACH(p, &m_peerq, ul_entry)
...
shuffle_optimists(void)
{
    for (int i = 0; i < m_npeers; i++) {
        struct peer *p = TOXNQ_FIRST(&m_peerq);
        if ((p->mp->flags & (PF_P_WANT|PF_I_CHOKE)) == (PF_P_WANT|PF_I_CHOKE)) {
            break;
        } else {
            TOXNQ_REMOVE(&m_peerq, p, ul_entry);
            TOXNQ_INSERT_TAIL(&m_peerq, p, ul_entry);
        }
    }
}
...
{
    toxn_timer_add(&m_choke_timer, CHOKE_INTERVAL);
...
ul_on_new_peer(struct peer *p)
{
    ...
    if (where < 1)
        TOXNQ_INSERT_HEAD(&m_peerq, p, ul_entry);
    else {
        struct peer *it = TOXNQ_FIRST(&m_peerq);
        where--;
        while (where > 0) {
            it = TOXNQ_NEXT(it, ul_entry);
            where--;
        }
        TOXNQ_INSERT_AFTER(&m_peerq, it, p, ul_entry);
    }
    m_npeers++;
    choke_do();
}
