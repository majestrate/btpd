...
{
    if (treq->sd != -1) {
        toxn_ev_del(&treq->ioev);
        close(treq->sd);
    }
    toxn_timer_del(&treq->timer);
    iobuf_free(&treq->buf);
    free(treq);
}
...
maybe_connect_to(struct torrent *tp, const char *pinfo)
{
    ...
    if (bcmp(toxn_get_peer_id(), pid, 20) == 0)
        return;
...
httptr_io_cb(int sd, short type, void *arg)
{
    ...
    case EV_READ:
        if (http_read(treq->req, sd) && !http_want_read(treq->req))
            toxn_ev_disable(&treq->ioev, EV_READ);
        break;
    case EV_WRITE:
        if (http_write(treq->req, sd) && !http_want_write(treq->req))
            toxn_ev_disable(&treq->ioev, EV_WRITE);
        break;
    case EV_TIMEOUT:
...
httptr_nc_cb(void *arg, int error, int sd)
{
    ...
    if (error) {
        ...
        toxn_ev_new(&treq->ioev, sd, flags, httptr_io_cb, treq);
        toxn_timer_add(&treq->timer, (& (struct timespec) { 30, 0 }));
    }
}
    ...
    enum tr_event event)
{
    ...
    const uint8_t *peer_id = toxn_get_peer_id();
    ...
    struct httptr_req *treq = toxn_calloc(1, sizeof(*treq));
    if (!http_get(&treq->req, url, "User-Agent: " TOXN_VERSION "\r\n",
            http_cb, treq)) {
        free(treq);
        return NULL;
    }
    ...
    treq->nc = toxn_name_connect(http_url->host, http_url->port,
        httptr_nc_cb, treq);
    evtimer_init(&treq->timer, httptr_io_cb, treq);
    toxn_timer_add(&treq->timer, (& (struct timespec) { 60, 0 }));
    return treq;
}
...
{
    if (treq->sd == -1)
        toxn_name_connect_cancel(treq->nc);
    http_cancel(treq->req);
    httptr_free(treq);
}
