...
nc_write_cb(int sd, short type, void *arg)
{
    ...
    if (getsockopt(nc->sd, SOL_SOCKET, SO_ERROR, &error, &errsiz) < 0)
        toxn_err("getsockopt error (%s).\n", strerror(errno));
    if (error == 0) {
        toxn_ev_del(&nc->write_ev);
        nc_done(nc, 0);
    } else {
        toxn_ev_del(&nc->write_ev);
        close(nc->sd);
...
again:
    ...
    if (err == 0)
        ...
        toxn_ev_new(&nc->write_ev, nc->sd, EV_WRITE, nc_write_cb, nc);
    else {
        close(nc->sd);
...
nameconn_t
toxn_name_connect(const char *name, short port, void (*cb)(void *, int, int),
    void *arg)
{
    struct addrinfo hints;
    struct nameconn *nc = toxn_calloc(1, sizeof(*nc));
    nc->cb = cb;
    nc->arg = arg;
    nc->sd = -1;
    ...
    nc->ai_handle = toxn_addrinfo(name, port, &hints, nc_ai_cb, nc);
    return nc;
}

void
toxn_name_connect_cancel(struct nameconn *nc)
{
    if (nc->state == NC_AI)
        toxn_addrinfo_cancel(nc->ai_handle);
    else {
        toxn_ev_del(&nc->write_ev);
        close(nc->sd);
    }
