...
{
    struct tlib *tl = toxn_calloc(1, sizeof(*tl));
    char hex[SHAHEXSIZE];
    bin2hex(hash, hex, 20);
    tl->num = m_nextnum;
...
load_info(struct tlib *tl, const char *path)
{
    ...
    if (read_file(path, buf, &size) == NULL) {
        toxn_log(TOXN_L_ERROR, "couldn't load '%s' (%s).\n", path,
            strerror(errno));
        return;
    }

    if (!valid_info(buf, size)) {
        toxn_log(TOXN_L_ERROR, "bad info file '%s'.\n", path);
        return;
    }
...
write_torrent(const char *mi, size_t mi_size, const char *path)
{
    ...
    return;
err:
    toxn_err("failed to write metainfo '%s' (%s).\n", path, strerror(errno));
}
    ...
    const char *content, char *name, char *label)
{
    ...
    if (mkdir(file, 0777) != 0)
        toxn_err("failed to create dir '%s' (%s).\n", file, strerror(errno));
...
tlib_init(void)
{
    ...
    m_hashtbl = hashtbl_create(1, toxn_id_eq, toxn_id_hash);
    if (m_numtbl == NULL || m_hashtbl == NULL)
        toxn_err("Out of memory.\n");

    if ((dirp = opendir("torrents")) == NULL)
        toxn_err("couldn't open the torrents directory.\n");
...
tlib_read_hash(struct tlib *tl, size_t off, uint32_t piece, uint8_t *hash)
{
    ...
    if ((errno = vopen(&fd, O_RDONLY, "torrents/%s/torrent", relpath)) != 0)
        toxn_err("failed to open 'torrents/%s/torrent' (%s).\n",
            relpath, strerror(errno));
    lseek(fd, off + piece * 20, SEEK_SET);
    if ((nread = read(fd, hash, 20)) != 20) {
        if (nread == -1)
            toxn_err("failed to read 'torrents/%s/torrent' (%s).\n", relpath,
                strerror(errno));
        else
            toxn_err("corrupt file: 'torrents/%s/torrent'.\n", relpath);
    }
...
init_resume(int fd, size_t size)
{
    ...
    return;
fatal:
    toxn_err("failed to initialize resume file (%s).\n", strerror(errno));
}
    ...
    size_t bfsize)
{
    ...
    char relpath[RELPATH_SIZE];
    struct stat sb;
    struct resume_data *resd = toxn_calloc(1, sizeof(*resd));
    bin2hex(tl->hash, relpath, 20);
    ...
    return resd;
fatal:
    toxn_err("file operation failed on 'torrents/%s/resume' (%s).\n",
        relpath, strerror(errno));
}
