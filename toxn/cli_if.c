...
cmd_tget(struct cli *cli, int argc, const char *args)
{
    ...
    opts = toxn_calloc(nkeys, sizeof(*opts));
    ...
    for (tl = tlib_iter_first(&it); tl != NULL; tl = tlib_iter_next(&it)) {
        if (torrent_startable(tl)) {
            if ((last_code = torrent_start(tl)) == IPC_OK) {
                ...
                toxn_err("torrent_start(%d) failed.\n", tl->num);
                ret_code = last_code;
            }
        }
    }
...
cmd_die(struct cli *cli, int argc, const char *args)
{
    ...
    if (!toxn_is_stopping()) {
        toxn_log(TOXN_L_TOXN, "Someone wants me dead.\n");
        toxn_shutdown();
    }
    return err;
}
...
client_connection_cb(int sd, short type, void *arg)
{
    ...
    if ((nsd = accept(sd, NULL, NULL)) < 0) {
        if (errno == EWOULDBLOCK || errno == ECONNABORTED)
            ...
            toxn_err("client accept: %s\n", strerror(errno));
    }

    if ((errno = set_blocking(nsd)) != 0)
        toxn_err("set_blocking: %s.\n", strerror(errno));

    struct cli *cli = toxn_calloc(1, sizeof(*cli));
    cli->sd = nsd;
    toxn_ev_new(&cli->read, cli->sd, EV_READ, cli_read_cb, cli);
}

void
ipc_shutdown(void)
{
    toxn_ev_del(&m_cli_incoming);
    close(m_listen_sd);
}
...
ipc_init(void)
{
    ...
    if (snprintf(addr.sun_path, psiz, "%s/sock", toxn_dir) >= psiz)
        toxn_err("'%s/sock' is too long.\n", toxn_dir);
    ...
    toxn_ev_new(&m_cli_incoming, sd, EV_READ, client_connection_cb, NULL);
    m_listen_sd = sd;
}
