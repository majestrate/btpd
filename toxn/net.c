...
struct peer_tq net_bw_readq = TOXNQ_HEAD_INITIALIZER(net_bw_readq);
struct peer_tq net_bw_writeq = TOXNQ_HEAD_INITIALIZER(net_bw_writeq);
struct peer_tq net_unattached = TOXNQ_HEAD_INITIALIZER(net_unattached);
...
net_ban_peer(struct net *n, struct meta_peer *mp)
{
    ...
    mp->flags |= PF_BANNED;
    toxn_log(TOXN_L_BAD, "banned peer %p.\n", mp);
}
...
{
    struct net *n = toxn_calloc(1, sizeof(*n));
    n->tp = tp;
    tp->net = n;

    if ((n->mptbl = mptbl_create(3, toxn_id_eq, toxn_id_hash)) == NULL)
        toxn_err("Out of memory.\n");

    TOXNQ_INIT(&n->getlst);

    n->busy_field = toxn_calloc(ceil(tp->npieces / 8.0), 1);
    n->piece_count = toxn_calloc(tp->npieces, sizeof(*n->piece_count));
}
...
net_stop(struct torrent *tp)
{
    ...
    struct piece *pc;
    while ((pc = TOXNQ_FIRST(&n->getlst)) != NULL)
        piece_free(pc);
    TOXNQ_INIT(&n->getlst);

    struct peer *p = TOXNQ_FIRST(&net_unattached);
    while (p != NULL) {
        struct peer *next = TOXNQ_NEXT(p, p_entry);
        if (p->n == n)
            peer_kill(p);
        p = next;
    }

    p = TOXNQ_FIRST(&n->peers);
    while (p != NULL) {
        struct peer *next = TOXNQ_NEXT(p, p_entry);
        peer_kill(p);
        p = next;
...
net_write(struct peer *p, unsigned long wmax)
{
    ...
    niov = 0;
    assert((nl = TOXNQ_FIRST(&p->outq)) != NULL);
    ...
    while ((niov < IOV_MAX && nl != NULL
        ...
        if (nl->nb->type == NB_PIECE) {
            if (block_count >= BLOCK_MEM_COUNT)
                break;
            struct net_buf *tdata = TOXNQ_NEXT(nl, entry)->nb;
            if (tdata->buf == NULL) {
    ...
    if (nwritten < 0) {
        if (errno == EAGAIN) {
            ...
            toxn_log(TOXN_L_CONN, "write error: %s\n", strerror(errno));
            peer_kill(p);
            return 0;
        }
    } else if (nwritten == 0) {
        toxn_log(TOXN_L_CONN, "connection closed by peer.\n");
        peer_kill(p);
        return 0;
    }
    ...
    case MSG_REQUEST:
        if ((p->mp->flags & (PF_P_WANT|PF_I_CHOKE)) == PF_P_WANT) {
            ...
            if ((length > PIECE_BLOCKLEN
                ...
                toxn_log(TOXN_L_MSG, "bad request: (%u, %u, %u) from %p\n",
...
net_state(struct peer *p, const char *buf)
{
    ...
    case SHAKE_ID:
        if ((net_torrent_has_peer(p->n, buf)
             || bcmp(buf, toxn_get_peer_id(), 20) == 0))
            goto bad;
    ...
    return 0;
bad:
    toxn_log(TOXN_L_CONN, "bad data from %p (%u, %u, %u).\n",
...
net_read(struct peer *p, unsigned long rmax)
{
    ...
    if (nread < 0 && errno == EAGAIN)
        ...
        toxn_log(TOXN_L_CONN, "Read error (%s) on %p.\n", strerror(errno), p);
        peer_kill(p);
        return 0;
    } else if (nread == 0) {
        toxn_log(TOXN_L_CONN, "Connection closed by %p.\n", p);
        peer_kill(p);
        return 0;
    }
    ...
    if (iov[1].iov_len > 0) {
        ...
        p->in.buf = toxn_malloc(p->in.st_bytes);
...
net_connect_addr(int family, struct sockaddr *sa, socklen_t salen, int *sd)
{
    ...
    if (connect(*sd, sa, salen) == -1 && errno != EINPROGRESS) {
        int err = errno;
        toxn_log(TOXN_L_CONN, "Botched connection %s.\n", strerror(errno));
        close(*sd);
        return err;
    }
...
net_connection_cb(int sd, short type, void *arg)
{
    ...
    toxn_log(TOXN_L_CONN, "got connection.\n");
}
...
compute_rates(void) {
    ...
    struct torrent *tp;
    TOXNQ_FOREACH(tp, torrent_get_all(), entry) {
        unsigned long tp_up = 0, tp_dwn = 0;
        struct net *n = tp->net;
        struct peer *p;
        TOXNQ_FOREACH(p, &n->peers, p_entry) {
...
net_bw_tick(void)
{
    ...
    if (net_bw_limit_in > 0) {
        while ((p = TOXNQ_FIRST(&net_bw_readq)) != NULL && m_bw_bytes_in > 0) {
            TOXNQ_REMOVE(&net_bw_readq, p, rq_entry);
            toxn_ev_enable(&p->ioev, EV_READ);
            p->mp->flags &= ~PF_ON_READQ;
            m_bw_bytes_in -= net_read(p, m_bw_bytes_in);
        }
    } else {
        while ((p = TOXNQ_FIRST(&net_bw_readq)) != NULL) {
            TOXNQ_REMOVE(&net_bw_readq, p, rq_entry);
            toxn_ev_enable(&p->ioev, EV_READ);
            p->mp->flags &= ~PF_ON_READQ;
            net_read(p, 0);
    ...
    if (net_bw_limit_out) {
        while (((p = TOXNQ_FIRST(&net_bw_writeq)) != NULL
                   && m_bw_bytes_out > 0)) {
...
run_peer_ticks(void)
{
    ...
    TOXNQ_FOREACH_MUTABLE(p, &net_unattached, p_entry, next)
        peer_on_tick(p);

    TOXNQ_FOREACH(tp, torrent_get_all(), entry)
        TOXNQ_FOREACH_MUTABLE(p, &tp->net->peers, p_entry, next)
        peer_on_tick(p);
}
...
net_read_cb(struct peer *p)
{
    if (net_bw_limit_in == 0)
        ...
        toxn_ev_disable(&p->ioev, EV_READ);
        p->mp->flags |= PF_ON_READQ;
        TOXNQ_INSERT_TAIL(&net_bw_readq, p, rq_entry);
    }
}
...
net_write_cb(struct peer *p)
{
    if (net_bw_limit_out == 0)
        ...
        toxn_ev_disable(&p->ioev, EV_WRITE);
        p->mp->flags |= PF_ON_WRITEQ;
        TOXNQ_INSERT_TAIL(&net_bw_writeq, p, wq_entry);
    }
}
...
net_shutdown(void)
{
    for (int i = 0; i < m_nlisteners; i++) {
        toxn_ev_del(&m_net_listeners[i].ev);
...
net_init(void)
{
    ...
    net_ipv6 = found_ipv6;
    if (!net_ipv4 && !net_ipv6)
        toxn_err("no usable address found. wrong use of -4/-6 perhaps.\n");
    m_nlisteners = count;
    m_net_listeners = toxn_calloc(count, sizeof(*m_net_listeners));
    for (ai = res; ai != NULL; ai = ai->ai_next) {
        count--;
        if ((sd = socket(ai->ai_family, ai->ai_socktype, 0)) == -1)
            toxn_err("failed to create socket (%s).\n", strerror(errno));
        ...
        if (bind(sd, ai->ai_addr, ai->ai_addrlen) == -1)
            toxn_err("bind failed (%s).\n", strerror(errno));
        listen(sd, 10);
        set_nonblocking(sd);
        m_net_listeners[count].sd = sd;
        toxn_ev_new(&m_net_listeners[count].ev, sd, EV_READ,
