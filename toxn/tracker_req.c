...
struct tr_entry {
    TOXNQ_ENTRY(tr_entry) entry;
    char *failure;
    char *url;
    enum tr_type type;
};

TOXNQ_HEAD(tr_entry_tq, tr_entry);
...
{
    struct tr_entry *e;
    TOXNQ_FOREACH(e, &t->trackers, entry)
        if (e->failure == NULL)
            return e;
    abort();
}
...
{
    toxn_log(TOXN_L_TR, "sending event %d to '%s' for '%s'.\n",
...
entry_send(struct tr_tier *t, struct tr_entry *e, enum tr_event event)
{
    ...
    t->event = event;
    t->cur = e;
    if (m_tlast_req > toxn_seconds - REQ_DELAY) {
        m_tnext_req = max(m_tnext_req, m_tlast_req) + REQ_DELAY;
        toxn_timer_add(&t->timer,
            (& (struct timespec) { m_tnext_req - toxn_seconds, 0 }));
        return;
    }
    toxn_timer_del(&t->timer);
    if ((t->req = req_send(t)) == NULL)
        toxn_err("failed to create tracker message to '%s' (%s).",
            e->url, strerror(errno));
    m_tlast_req = toxn_seconds;
}
...
tier_start(struct tr_tier *t)
{
    ...
    t->bad_conns = 0;
    t->active = 1;
    entry_send(t, TOXNQ_FIRST(&t->trackers), TR_EV_STARTED);
}
...
tier_complete(struct tr_tier *t)
{
    if (tier_active(t) && t->event == TR_EV_EMPTY)
        entry_send(t, TOXNQ_FIRST(&t->trackers), TR_EV_COMPLETED);
}
...
add_tracker(struct tr_tier *t, const char *url)
{
    ...
    if ((hu = http_url_parse(url)) != NULL) {
        http_url_free(hu);
        e = toxn_calloc(1, sizeof(*e));
        if ((e->url = strdup(url)) == NULL)
            toxn_err("Out of memory.\n");
        e->type = TR_HTTP;
    } else {
        toxn_log(TOXN_L_TR, "skipping unsupported tracker '%s' for '%s'.\n",
            url, torrent_name(t->tp));
        return;
    }
    TOXNQ_INSERT_TAIL(&t->trackers, e, entry);
}

static struct tr_tier *
tier_create(struct torrent *tp, struct mi_tier *tier)
{
    struct tr_tier *t = toxn_calloc(1, sizeof(*t));
    t->tp = tp;
    TOXNQ_INIT(&t->trackers);
    for (int i = 0; i < tier->nurls; i++)
        add_tracker(t, tier->urls[i]);
    if (!TOXNQ_EMPTY(&t->trackers)) {
        t->interval = -1;
...
tier_kill(struct tr_tier *t)
{
    ...
    if (t->req != NULL)
        req_cancel(t);
    TOXNQ_FOREACH_MUTABLE(e, &t->trackers, entry , next) {
        if (e->failure != NULL)
            free(e->failure);
        free(e->url);
        free(e);
...
tr_create(struct torrent *tp, const char *mi)
{
    ...
    tp->tr = toxn_calloc(1, sizeof(*tp->tr));
    TOXNQ_INIT(&tp->tr->trackers);
    if ((ann = mi_announce(mi)) == NULL)
        toxn_err("Out of memory.\n");
    for (i = 0; i < ann->ntiers; i++)
        if ((t = tier_create(tp, &ann->tiers[i])) != NULL)
            TOXNQ_INSERT_TAIL(&tp->tr->trackers, t, entry);
...
tr_kill(struct torrent *tp)
{
    ...
    TOXNQ_FOREACH_MUTABLE(t, &tp->tr->trackers, entry, next)
        tier_kill(t);
    free(tp->tr);
    tp->tr = NULL;
}
...
{
    struct tr_tier *t;
    TOXNQ_FOREACH(t, &tp->tr->trackers, entry)
        tier_start(t);
}
...
tr_result(struct tr_tier *t, struct tr_response *res)
{
    ...
    case TR_RES_FAIL:
        ...
        toxn_log(TOXN_L_ERROR, "tracker at '%s' failed (%s).\n",
    ...
    case TR_RES_CONN:
        toxn_log(TOXN_L_TR, "connection to '%s' failed for '%s'.\n",
            t->cur->url, torrent_name(t->tp));
        e = t->cur;
        while ((e = TOXNQ_NEXT(e, entry)) != NULL && e->failure != NULL)
            ;
        if (e != NULL) {
        ...
        if (t->event == TR_EV_STOPPED && t->bad_conns > 1)
            ...
            entry_send(t, TOXNQ_FIRST(&t->trackers), t->event);
        else if (t->bad_conns == 2)
            toxn_timer_add(&t->timer, RETRY1_TIMEOUT);
        else
            toxn_timer_add(&t->timer, RETRY2_TIMEOUT);
        break;
    case TR_RES_BAD:
        toxn_log(TOXN_L_ERROR, "bad data from tracker '%s' for '%s'.\n",
            t->cur->url, torrent_name(t->tp));
    case TR_RES_OK:
        if (TR_RES_OK)
            toxn_log(TOXN_L_TR, "response from '%s' for '%s'.\n",
                t->cur->url, torrent_name(t->tp));
        if (t->event == TR_EV_STOPPED)
            ...
            toxn_timer_add(&t->timer, (& (struct timespec) {
                t->interval > 0 ? t->interval : DEFAULT_INTERVAL, 0 }));
        }
        t->bad_conns = 0;
        t->has_responded = 1;
        TOXNQ_REMOVE(&t->trackers, t->cur, entry);
        TOXNQ_INSERT_HEAD(&t->trackers, t->cur, entry);
        break;
    default:
        abort();
    }
