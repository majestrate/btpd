...
enum ipc_err
toxn_tget(struct ipc *ipc, struct ipc_torrent *tps, size_t ntps,
...
enum ipc_err
toxn_tget_wc(struct ipc *ipc, enum ipc_twc twc, enum ipc_tval *keys,
...
enum ipc_err
toxn_add(struct ipc *ipc, const char *mi, size_t mi_size, const char *content,
...
enum ipc_err
toxn_del(struct ipc *ipc, struct ipc_torrent *tp)
{
    return simple_treq(ipc, "del", tp);
}

enum ipc_err
toxn_rate(struct ipc *ipc, unsigned up, unsigned down)
{
    struct iobuf iob = iobuf_init(32);
...
enum ipc_err
toxn_start(struct ipc *ipc, struct ipc_torrent *tp)
{
    return simple_treq(ipc, "start", tp);
}

enum ipc_err
toxn_start_all(struct ipc *ipc)
{
    struct iobuf iob = iobuf_init(16);
...
enum ipc_err
toxn_stop(struct ipc *ipc, struct ipc_torrent *tp)
{
    return simple_treq(ipc, "stop", tp);
}

enum ipc_err
toxn_stop_all(struct ipc *ipc)
{
    struct iobuf iob = iobuf_init(16);
